...
\title{A Hybrid Quantum-Classical Computational
Study of Energetic and Conformational Differences in Amyloid-$\beta$ variants}

\author{Kushal Patil}
\affiliation{
 Lincoln Sudbury Regional High School, 390 Lincoln Road, Sudbury, MA 01776, USA
}
...
\begin{abstract}
A hybrid quantum-classical approach with all-atom MD and Variational Quantum Eigensolver (VQE) is applied to analyze electronic level differences between Amyloid-$\beta$ variants that influence mutation-driven protein aggregation in Alzheimer's Disease. Alzheimer’s disease, the most common form of dementia, affects seven million people in the US alone. A defining molecular hallmark of the disease is the accumulation of extracellular plaques composed of beta-amyloid (A$\beta$). In particular, the Arctic variant of A$\beta$ exhibits accelerated aggregation compared to wild-type peptides. Computational approaches, such as all-atom molecular dynamics (MD) simulations, remain limited in their ability to look into the quantum-level electronic properties that underpin the aggregation propensity. (1) Molecular dynamics simulations are performed to sample wild-type and Arctic amyloid-$\beta$ structures. (2) Stable LVFFA fragments were extracted, capped, and analyzed with DFT. (3) Active-space Hamiltonians were mapped to qubits for Variational Quantum Eigensolver (VQE) simulations. (4) Electronic energy differences between mutant and wild-type variants were calculated, then validated with experimental data. This workflow links long-timescale MD sampling to quantum electronic structure analysis, targeting mutation-induced aggregation effects in neurodegenerative disease.  The Arctic variant displayed more compact $\beta$-sheet-rich conformations in MD, consistent with aggressive protein aggregation. Quantum-calculated energy gaps between wild-type and mutant fragments correlated with HOMO-LUMO shifts, suggesting electronic-level mutation effects. DFT/RHF along with experimental data confirmed quantum simulations’ accuracy. These results demonstrate that quantum electronic descriptors capture the mutation’s impact and support the computational approach’s relevance for studying neurodegeneration. This suggests a direct quantum-level contribution to its propensity for aggregation. Our results demonstrate a transferable workflow for probing biomolecular electronic structure on near-term quantum devices, bridging molecular biophysics and quantum computation for larger systems.
\end{abstract}

\keywords{VQE, Amyloid Beta, Wild-type, Arctic}
\maketitle
...
Here, I propose a combined classical–quantum workflow that integrates MD simulations with a quantum algorithm --- a Variational Quantum Eigensolver (VQE) \cite{peruzzo2014} --- to examine electronic structure features of A$\beta$ peptide fragments, specifically the hydrophobic LVFFA core (Figure \ref{fig:VLFFA}). This hybrid approach aims to accelerate quantum simulation workflows while offering deeper mechanistic insights and paving the way for broader applications in the study of protein misfolding and other complex biophysical systems.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\linewidth]{LVFFA.png}
    \caption{Wild Type Amyloid beta with LVFFA core highlighted}
    \label{fig:VLFFA}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{Overlays of the 3 sets of snapshots ranked from least to most energetic (Set 1, Set 2, Set 3).}
    \label{fig:SET123}
\end{figure}
...
\subsection{Electronic Structure Problem Creation}

 Electronic structure problems were prepared using PySCF 2.7.0. The PySCF driver was a dual-path driver function that integrates classical solvation effects using ddCOSMO ($\epsilon$=80.0). Each fragment was first modeled using PySCF’s restricted Hartree-Fock (RHF) method, followed by ddCOSMO to simulate an aqueous environment. This structure captures key electrostatic effects relevant to biological environments. The molecular geometry, charge, and spin multiplicity were specified with Angstroms and STO-3G as the basis set. A shift of 0.2 was added to aid SCF convergence. Selected HOMO-LUMO orbitals \st{from} were used to generate Gaussian cube files; prior energy identification runs were used to identify these orbitals(tables in supplement). To interface with the Qiskit Quantum computing program \cite{qiskit2019,qiskit_nature2021}, Qiskit-(0.7.2)’s PySCFDriverNature\cite{pyscf2020} was used to create an ElectronicStructureProblem object. This object will be mapped and used for the VQE run \cite{peruzzo2014}.

\subsection{Active Space Creation}
 
To reduce computational overhead and focus on chemically relevant orbitals, Qiskit Nature’s ActiveSpaceTransformer was employed to construct reduced Hamiltonians for VQE simulations.  The active space configuration  \cite{active_space} explored was 4 orbitals/4 electrons (4o4e). These spaces were centered around the frontier orbitals, specifically the HOMO–LUMO region, with symmetric extensions of ±1 orbitals to capture near-degenerate excitations and charge-transfer character.
The active space was defined by selecting molecular orbitals relative to the canonical HOMO and LUMO indices obtained from PySCF. This approach ensures that the most chemically and electronically significant orbitals are retained, while discarding deeply core and high-lying virtual orbitals that contribute minimally to correlation energy. The resulting second-quantized Hamiltonians were mapped to qubit operators using the Jordan-Wigner mapper. This strategy balances physical accuracy with quantum resource efficiency, enabling meaningful comparisons between Arctic and wild-type A$\beta$ fragments across conformational ensembles.

\subsection{Quantum Simulation Protocol}

Molecular integrals from each active space configuration were generated using PySCF and down-folded into second-quantized Hamiltonians. These were mapped to qubit operators via the Jordan–Wigner transformation using Qiskit-nature 0.7.2. Simulations targeted the LVFFA segment of Arctic and wild-type conformers, extracted from MD trajectories and capped with hydrogen atoms.
...
\begin{figure*}
    ...
    \caption{UCCSD Ansatz circuit diagram (Depth 196, 8 Qubits)}
    \label{fig:UCCSD}
\end{figure*}
...
\begin{figure*}
    ...
    \caption{TwoLocal Ansatz circuit diagram (Depth 166, 8 Qubits)}
    \label{fig:placeholder}
\end{figure*}


All quantum simulations were performed on the Qiskit state vector simulator back end. Total VQE energies were computed for Arctic and wild-type fragments across each active space configuration. The resulting energy gap ($\Delta$E) was used to quantify electronic stability differences and assess the impact of mutation on the energetics of the ground state.
...
Experimental studies using thioflavin T fluorescence and kinetic modeling showed Arctic mutation increases the secondary nucleation rate by over 100-fold compared to wild-type\cite{exp}. In cell culture, Arctic aggregates appear within 24 hours, while wild-type show minimal aggregation. Our quantum calculations provide the electronic explanation: the E22G mutation creates conformation-dependent stabilization, making compact, aggregation-prone structures up to 0.05 Hartrees more stable. This electronic preference directly drives the mutation toward states that accelerate fibril surface nucleation - exactly matching the experimental 100-fold rate enhancement. The quantum calculations reveal why the mutation aggregates faster at the electron level.

                \section{Results}
        \subsection{\textbf{All-atom MD}}

To probe the structural landscape, all-atom MD for wild-type and Arctic Amyloid beta was performed. From the production trajectories, potential energy profiles were extracted using gmx energy. Conformational Stability was assessed using the resulting XVG graphs with pronounced energy fluctuations, consistent with transient stabilization events. Rather than relying on a single snapshot, I extracted the top three lowest-energy conformers from each trajectory using gmx trjconv -dump (Figure 6), ensuring coverage of the most populated basin. These conformers were capped with hydrogen atoms to yield chemically complete LVFFA fragments suitable for quantum analysis. Gmx dssp and clustering (Figures 7 and 8) confirmed that the selected Arctic conformers were structurally compact and $\beta$-sheet–enriched, while wild-type conformers displayed greater backbone flexibility and coil-like features.


\begin{figure}
\includegraphics[width=0.48\linewidth]{AVPT.png}
\includegraphics[width=0.48\linewidth]{WTPT.png}
\caption{Time evolution of the potential energy during molecular dynamics simulations of the amyloid-beta peptide. The graph displays the total potential energy(kJ/mol) as a function of simulation time for both wild-type and Arctic variants (E22G) (WT on right, Arctic on left)}
\end{figure}

\begin{figure}
\includegraphics[width=0.48\linewidth]{SSA.png}
\includegraphics[width=0.48\linewidth]{SSWT_yaxisupdate.png}
\caption{Secondary structure assignment (DSSP) over the course of molecular dynamics simulations for wild-type and Arctic (E22G) amyloid-beta variants. (Arctic on right, Wild Type on left)}
\end{figure} 
...
Each capped conformer was subjected to geometry optimization and orbital analysis via PySCF. HOMO–LUMO gaps and orbital localization patterns were consistent across Arctic conformers, supporting their electronic stability. These fragments were then used for downstream DFT and VQE simulations, enabling ensemble-level comparison of electronic energies and mutation-induced stabilization.

\subsection{\textbf{Electronic Structure and Active Space Selection}}

Molecular orbital analysis of each representative snapshot revealed well-defined frontier orbitals localized predominantly on the LVFFA core. An active space spanning four spatial orbitals was selected centered around the HOMO–LUMO pair to capture key electronic correlations. Orbital isosurface plots comparing Arctic and wild-type variants (Figure 9) illustrate subtle but meaningful changes in electronic density distribution induced by the E22G mutation, supporting the rationale for active space targeting.
...
Set 3
\end{center}
\caption{Visualization of HOMO-LUMO +/-1 Gaussian cube files depicting orbital isosurface amplitude from set 1. Generated by PySCF[11], visualized by ChimeraX[9], isovalue +/- 0.01 au, Red indicates the negative orbital surface, Blue indicates the positive orbital surface.}
\end{figure}

\subsection{\textbf{Variational Quantum Eigensolver Convergence and Robustness}}

VQE calculations employing a TwoLocal ansatz with COBYLA optimization converged reliably across all three representative snapshots, as shown by energy convergence curves (Figures 10 and 11). The ansatz circuit depth and qubit count were sufficient to capture the essential correlation effects within the active space. Preliminary tests with alternative ansatzes, ie. UCCSD further confirms that the observed results are robust concerning the choice of quantum circuit architecture.

\begin{figure}
\includegraphics[width=0.75\linewidth]{SET1TLC.png}
\begin{center}
Set 1
\end{center}
\includegraphics[width=0.75\linewidth]{SET2TLC.png}
\begin{center}
Set 2
\end{center}
\includegraphics[width=0.75\linewidth]{SET3TLC.png}
\begin{center}
Set 3
\end{center}
\caption{VQE convergence plots for the TwoLocal Ansatz with COBYLA optimizer}
\end{figure}

\begin{figure} 
\includegraphics[width=0.75\linewidth]{set1ucc.png}
\begin{center}
Set 1
\end{center}
\includegraphics[width=0.75\linewidth]{set2ucc.png}
\begin{center}
Set 2
\end{center}
\includegraphics[width=0.75\linewidth]{set3ucc.png}
\begin{center}
Set 3
\end{center}
...
\begin{table}
    ...
    \begin{tabular}{lccc}
        \hline
        \textbf{TwoLocal} & \textbf{set 1} & \textbf{set 2} & \textbf{set 3} \\
        \hline
...
\begin{table}    
    ...
    \begin{tabular}{lccc}
        \hline
        \textbf{UCCSD} & \textbf{set 1} & \textbf{set 2} & \textbf{set 3} \\
        \hline
